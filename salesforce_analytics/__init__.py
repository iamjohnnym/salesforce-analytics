...
class SalesForceAnalytics(object):
    ...
    def __build_counts(self):
        for key in self.tickets[0].keys():
            if 'attribute' not in key:
                self.count.update({self.__set_json_compliant_name(key): {}})
        return self.count

    def __build_trends(self):
        for key in self.trend_topics:
            self.trend.update({self.__set_json_compliant_name(key): {}})
        return self.trend
    ...
    def __add_key(self, key):
        if key not in self.count:
            self.count[self.__set_json_compliant_name(key)] = {}
    ...
    def __set_count_keys(self, key, value):
        ...
        value of 0"""
        if value not in self.count[self.__set_json_compliant_name(key)]:
            self.count[self.__set_json_compliant_name(key)][value] = 0
        self.count[self.__set_json_compliant_name(key)][value] += 1

    def __set_mtc_times(self, key, value, ticket):
        name = self.__set_json_compliant_name(key)
        if name not in self.mtc_time:
            self.mtc_time.update({name: {}})
        if value not in self.mtc_time:
            self.mtc_time[name].update({value: {}})

    def __set_json_compliant_name(self, salesforce_field):
    ...
    def describe_count(self, key, value):
        ...
        """
        key = self.__set_json_compliant_name(key)
